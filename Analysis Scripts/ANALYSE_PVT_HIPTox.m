...
RT_cut = 0; % Remove all trials less than 0 ms, indicating anticipation error
RT_SDs = 100; % Number of standard deviations to cut RTs by
...
RespMat{1,11} = 'Number of trial warnings';
RespMat{1,12} = 'Number of pauses';
...
for current_p = min(results_table.ParticipantID):max(results_table.ParticipantID) % i.e., For participant number 1:X
    for current_v = min(results_table.VisitNumber):max(results_table.VisitNumber) % i.e., For visit number 1:X
        for current_s = min(results_table.SessionNumber):max(results_table.SessionNumber) % i.e., For session number 1:X
            ...
            currentResults.accuracy = results_table.Response_(results_table.ParticipantID == current_p & results_table.VisitNumber == current_v & results_table.SessionNumber == current_s & results_table.RawRT > RT_cut);
            ...
            currentResults.trialLength = double(results_table.TrialType(results_table.ParticipantID == current_p & results_table.VisitNumber == current_v & results_table.SessionNumber == current_s & results_table.RawRT > RT_cut) == "Long");
            ...
            currentResults.RT(currentResults.RT > (mean(currentResults.RT(currentResults.trialLength == currentResults.trialLength & currentResults.trialResp == currentResults.trialResp))+ (RT_SDs*std(currentResults.RT(currentResults.trialLength == currentResults.trialLength & currentResults.trialResp == currentResults.trialResp)))) | currentResults.RT < (mean(currentResults.RT(currentResults.trialLength == currentResults.trialLength & currentResults.trialResp == currentResults.trialResp))-(RT_SDs*std(currentResults.RT(currentResults.trialLength == currentResults.trialLength & currentResults.trialResp == currentResults.trialResp))))) = NaN;
            ...
            RespMat{count+1,11} = warning_num;
            RespMat{count+1,12} = pause_num;
...
writetable(Data, 'PVT_Results_HIPTox.xlsx')
