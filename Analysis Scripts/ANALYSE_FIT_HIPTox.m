...
% function ANALYSE_FIT_HIPTox
%
%% Script created to analyse FIT and output associated metrics %%
%
% Dr Tom Faherty, 20th March 2024
% t.b.s.faherty@bham.ac.uk
%
% Provides average RT and Accurary for one-face trials and two-face trials under all congruency conditions
...
RespMat{1,14} = 'Number of trial warnings';
RespMat{1,15} = 'Number of pauses';
RespMat{1,16} = 'Number missed';
RespMat{1,17} = 'RT all trials';
...
for current_p = min(results_table.ParticipantID):max(results_table.ParticipantID) % i.e., For participant number 1:X
    for current_v = min(results_table.VisitNumber):max(results_table.VisitNumber) % i.e., For visit number 0:X
        for current_s = min(results_table.SessionNumber):max(results_table.SessionNumber) % i.e., For session number 1:2
            ...
            currentResults.block = results_table.BlockNumber(results_table.ParticipantID == current_p & results_table.VisitNumber == current_v & results_table.SessionNumber == current_s & results_table.RawRT > RT_cut);
            currentResults.blocktrial = results_table.BlockTrialNumber(results_table.ParticipantID == current_p & results_table.VisitNumber == current_v & results_table.SessionNumber == current_s & results_table.RawRT > RT_cut);
            currentResults.onetrialType = results_table.TrialType(results_table.ParticipantID == current_p & results_table.VisitNumber == current_v & results_table.SessionNumber == current_s & results_table.RawRT > RT_cut); % 1 = 1-face; 2 = 2-face
            currentResults.Response = results_table.Correct_(results_table.ParticipantID == current_p & results_table.VisitNumber == current_v & results_table.SessionNumber == current_s & results_table.RawRT > RT_cut); % 0 = Incorrect Response; 1 = Correct Response; 2 = No Response
            currentResults.Accuracy = double(results_table.Correct_(results_table.ParticipantID == current_p & results_table.VisitNumber == current_v & results_table.SessionNumber == current_s & results_table.RawRT > RT_cut) == 1); % 1 = Correct; 0 = Incorrect
            currentResults.RT = results_table.RawRT(results_table.ParticipantID == current_p & results_table.VisitNumber == current_v & results_table.SessionNumber == current_s & results_table.RawRT > RT_cut);
            ...
            miss_array = results_table.Response(results_table.ParticipantID == current_p & results_table.VisitNumber == current_v & results_table.SessionNumber == current_s);
            ...
            % Calculate RT for all trials

             all_RT = mean(onetrialResults.RT(onetrialResults.Accuracy == 1));
            ...
            RespMat{count+1,14} = warning_num;
            RespMat{count+1,15} = pause_num;
            RespMat{count+1,16} = miss_num;
            RespMat{count+1,17} = all_RT;
...
writetable(Data, 'FIT_Results_HIPTox_New.xlsx')
