...
% function ANALYSE_ERT_HIPTox
%
%% Script created to analyse ERT and output associated metrics %%
%
% Dr Tom Faherty, 4th December 2023
...
maxBlock = 5; % Change which blocks are included in the analyses
RT_cut = 200; % Remove all trials less than 250 ms, indicating anticipation error
RT_SDs = 2; % Number of standard deviations to cut RTs by
...
RespMat{1,13} = 'Number of warnings';
RespMat{1,14} = 'Number of pauses';
RespMat{1,15} = 'Computer used';
...
% Task switch = 0       First 8 trials in block (50/50 emotion expression)
...
results_table.ParticipantID = str2double(extract(results_table.ParticipantID, digitsPattern));

% Start loop

for current_p = min(results_table.ParticipantID):max(results_table.ParticipantID) % i.e., For participant number 1:X
    for current_v = min(results_table.VisitNumber):max(results_table.VisitNumber) % i.e., For visit number 1:X
        for current_s = min(results_table.SessionNumber):max(results_table.SessionNumber) % i.e., For session number 1:X
            ...
            current_v % Print current visit (So we know the code is running)
            ...
            computerTable = results_table.LaptopName(results_table.ParticipantID == current_p & results_table.VisitNumber == current_v & results_table.SessionNumber == current_s);

            if ~isempty(computerTable)
                computer = computerTable(1);
            end

            currentResults.block = results_table.BlockNumber(results_table.ParticipantID == current_p & results_table.VisitNumber == current_v & results_table.SessionNumber == current_s);
            currentResults.switchType = results_table.BlockTrialNumber(results_table.ParticipantID == current_p & results_table.VisitNumber == current_v & results_table.SessionNumber == current_s) > 8; % 0 = Switch, 1 = Main
            currentResults.targetType = results_table.TargetClassifier(results_table.ParticipantID == current_p & results_table.VisitNumber == current_v & results_table.SessionNumber == current_s);
            currentResults.signalResponse = results_table.FAHitCROrMiss_(results_table.ParticipantID == current_p & results_table.VisitNumber == current_v & results_table.SessionNumber == current_s);
            currentResults.RT = results_table.RawRT(results_table.ParticipantID == current_p & results_table.VisitNumber == current_v & results_table.SessionNumber == current_s);

            % Trial repeats (Doesn't matter if data is since deleted, still gives an indication of 'problem' data)

            repeat_find = results_table.Repeat_(results_table.ParticipantID == current_p & results_table.VisitNumber == current_v & results_table.SessionNumber == current_s);

            pause_find = find(strcmp('p',repeat_find));
            warning_find = find(strcmp('w',repeat_find));

            pause_num = size(pause_find,1);
            warning_num = size(warning_find,1);
            ...
            switchResults(switchResults.block < 2,:) = []; % Remove 1st block (as no switch occurs!)
            ...
            % Now we can force NaN for Hit & FA trials which are out of a specific range (i.e., 2.5 SDs away from trial type average)
            ...
            % Set up a loop to create d' and hit RT for each trial type
            ...
            % For simple discrimination, find overall d'

            hit_count = nnz(biasResults.signalResponse == 1);
            miss_count = nnz(biasResults.signalResponse == 3);
            FA_count = nnz(biasResults.signalResponse == 0);
            CR_count = nnz(biasResults.signalResponse == 2);
            ...
            overall_d_prime = norminv(CalcHitRate, 0, 1) - norminv(CalcFalseAlarmRate, 0, 1);
            ...
            RespMat{count+1,13} = warning_num;
            RespMat{count+1,14} = pause_num;
            RespMat{count+1,15} = computer;
...
writetable(Data, 'ERT_Results_HIPTox.xlsx')
