    ...
    for current_v = min(results_table.VisitNumber):max(results_table.VisitNumber) % i.e., For visit number 0:X
        for current_s = min(results_table.SessionNumber):max(results_table.SessionNumber) % i.e., For session number 1:2
            ...
            for current_n = min(results_table.n):max(results_table.n) % For each n
                ...
                % Find the results for this participant, session, visit, and n within the table

                % This no longer works for some reason... accuracy = double(results_table.Correct_((results_table.n == current_n * (results_table.SessionNumber == current_s * (results_table.VisitNumber == current_v * (results_table.ParticipantID == current_p * (results_table.Correct_ < 3)))))) == 1);
                % This no longer works for some reason... RT = results_table.RawRT((results_table.ParticipantID == current_p*(results_table.VisitNumber == current_v*(results_table.SessionNumber == current_s*(results_table.n == current_n*(results_table.Correct_ < 2))))));
                % This no longer works for some reason... trial_type = double(results_table.TrialType((results_table.ParticipantID == current_p*(results_table.VisitNumber == current_v*(results_table.SessionNumber == current_s*(results_table.n == current_n*(results_table.Correct_ < 3)))))) == 1);

                accuracy = results_table.Correct_(results_table.Correct_ < 3 & results_table.n == current_n & results_table.SessionNumber == current_s & results_table.VisitNumber == current_v & results_table.ParticipantID == current_p);
                RT = results_table.RawRT(results_table.Correct_ < 2 & results_table.n == current_n & results_table.SessionNumber == current_s & results_table.VisitNumber == current_v & results_table.ParticipantID == current_p);
                trial_type = results_table.TrialType(results_table.Correct_ < 3 & results_table.n == current_n & results_table.SessionNumber == current_s & results_table.VisitNumber == current_v & results_table.ParticipantID == current_p);
                
                missedtrials = sum(results_table.Correct_(results_table.Correct_ < 3 & results_table.n == current_n & results_table.SessionNumber == current_s & results_table.VisitNumber == current_v & results_table.ParticipantID == current_p) == 2);
                ...
                % Save data to associated participant, visit, and session
...
writetable(Data, 'SNB_Results_HIPTox.xlsx')
